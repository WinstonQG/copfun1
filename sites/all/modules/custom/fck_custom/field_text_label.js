(function($) {		
  	Drupal.behaviors.fieldTextLabel = {
      
	    attach: function(data, settings) {    		    	
	    	var typingTimer;
			var doneTypingInterval = 600;

	    	$('input[id*="field-text-label"]').on('keyup', function (e) {
			...
			$(document).ajaxComplete(function(event, request, settings) {			  			  
			  	...
			  	if (id !== undefined && id.indexOf('field_text_label') !== -1) {
			  		$('input[id*="field-text-label"]').focus();
			  	}			
			});
	    }
	};
