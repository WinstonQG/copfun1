(function ($) {
    ...
    FooterIcon = function (select, context) {
        ...
        this.createListIcon = function () {
            var listText = $.trim($('.follow-isocial', context).val()),
            ...
            $.each(listArray, function(index, value) {
                html += '<li class="icon-sort" id="li-'+index+'">'
                        ...
                        +'</div>'
                        +'<div class="wrap-icon">'
                        +'<a data-class="'+value[3]+'" data-name="'+value[1]+'" data-link="'+value[0]+'" data-value="'+value[2]+'" href="#"><i class="'+value[3]+'"></i></a>'
                        +'</div>'
                        +'</li>';
            })
        ...
        this.OpenDialog = function () {
            ...
            $(context).delegate('.wrap-icon', 'click', function() {
                var listId = $(this).parents('.icon-sort').attr('id'),
                    ...
                    iconName = $(this).find('a').data('name'),
                ...
                self.formDialog.find('.policy-name-icon').val(iconName);
            ...
            self.formDialog.dialog({
                close: function( event, ui ) {
                    var iconValue = self.formDialog.find('.choose-icon').attr('data-value'),
                        ...
                        iconName = self.formDialog.find('.policy-name-icon').val() || '#',
                        html = '<a data-class="'+iconClass+'" data-name="'+iconName+'" data-link="'+iconLink+'" data-value="'+ iconValue +'" href="#"><i class="'+iconClass+'"></i></a>';
        ...
        this.updateInput = function () {
            ...
            $('#sortable').find('.wrap-icon').find('a').each(function (){
                var iconLink = $(this).data('link'),
                    ...
                    icon = [iconLink, iconName, iconValue, iconClass];
                inputValue.push(icon);
            });
            $('.follow-isocial', context).val(inputValue.toString());
        }
    ...
    Drupal.behaviors.myBehavior = {
        attach: function (context, settings) {
            var runfooter = new FooterIcon('#edit-follow-icon', context);
            runfooter.init();
        }
    };
})(jQuery);